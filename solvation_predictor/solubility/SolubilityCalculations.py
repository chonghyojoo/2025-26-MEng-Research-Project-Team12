...
class SolubilityCalculations:
    ...
    def make_calculations_298(
        ...
        self.gsolv_aq_298, self.unc_gsolv_aq_298 = self.gsolv_aq_298[:, 0], self.unc_gsolv_aq_298[:, 0]
        if predictions.ghsolv:
            self.gsolv_298, self.unc_gsolv_298 = self.extract_predictions(predictions.ghsolv)
            self.gsolv_298, self.unc_gsolv_298 = self.gsolv_298[:, 0], self.unc_gsolv_298[:, 0]
        # if predictions.gsolv:
    ...
    def make_calculations_t(
        ...
        if predictions.ghsolv:
            self.hsolv_298, self.unc_hsolv_298 = self.extract_predictions(predictions.ghsolv)
            self.hsolv_298, self.unc_hsolv_298 = self.hsolv_298[:, 0], self.unc_hsolv_298[:, 0]
        if predictions.hsolv:
        ...
        self.V = np.array(
            [
                self.calculate_solute_parameter_v(solute)
                for solute in predictions.data.smiles_solutes
            ]
        )
        self.I_OHadj, self.I_OHnonadj, self.I_NH = self.get_diol_amine_ids(
            predictions.data.smiles_solutes
        )
        ...
        if calculate_t_dep_with_t_dep_hdiss:
            ...
            coolprop_name_list, crit_t_list, crit_d_list = self.get_solvent_info(
                predictions.data.smiles_solvents1, self.solv_crit_prop_dict
            )
    ...
    def get_diol_amine_ids(self, solute_smiles):
        unique_solutes = set(solute_smiles)
        dict_diol_amine = dict()
        for i in unique_solutes:
            dict_diol_amine[i] = self.get_individual_diol_amine_ids(i)
        I_OHadj = [1 if dict_diol_amine[i][0] else 0 for i in solute_smiles]
        I_OHnonadj = [1 if dict_diol_amine[i][1] else 0 for i in solute_smiles]
        I_NH = [1 if dict_diol_amine[i][2] else 0 for i in solute_smiles]
    ...
    def get_solvent_info(self, smiles_solvents, solv_crit_prop_dict):
        solvents_smiles_list = smiles_solvents
