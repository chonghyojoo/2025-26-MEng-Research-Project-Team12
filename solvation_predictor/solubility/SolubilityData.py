...
class SolubilityData:
    def __init__(
        ...
        """
        self.df = df
        self.smiles_solvents1 = []
        self.smiles_solvents2 = None
        self.smiles_solutes = []
        self.molefracs = []
    ...
    def get_data_from_df(self, verbose=False):
        ...
        self.smiles_solvents1 = self.df["solvent_smiles"].values.tolist()
        if "solvent2_smiles" in self.df.columns:
            self.smiles_solvents2 = self.df["solvent2_smiles"].values.tolist()
            self.molefracs = self.df["molefrac"].values.tolist()

        self.smiles_solutes = self.df["solute_smiles"].values.tolist()


        if verbose:
            self.logger("Reading dataframe")
        if not "solute_smiles" in self.df.columns:
        ...
        if self.validate:
            ...
            self.smiles_solvents1 = self.df["solvent_smiles"].values.tolist()
            if "solvent2" in self.df.columns:
                self.smiles_solvents2 = self.df["solvent2"].values.tolist()
            self.smiles_solutes = self.df["solute_smiles"].values.tolist()
    ...
    def validate_data(self, verbose=False):
        ...
        if "solute_smiles" in self.validate_data_list:
            self.df["input_solute"] = self.df["solute"]
            validate_solute, new_value_dict["solute"] = True, []
            cols_to_move.append("input_solute")
        if "solvent_smiles" in self.validate_data_list:
            self.df["input_solvent"] = self.df["solvent1"]
        ...
        for index, row in self.df.iterrows():
            ...
            if validate_solvent:
                solvent_smiles, error_msg = self.validate_smiles(
                    row.solvent1, error_msg, "solvent1"
                )
