...
def convert_arrays_to_df(
    ...
    """
    df = pd.DataFrame()
    if len(solvent_smiles) != len(solute_smiles):
        raise ValueError(
            "The number of solvent and solute smiles must be the same. "
            "You provided {} solvent smiles and {} solute smiles.".format(
                len(solvent_smiles), len(solute_smiles)
            )
        )
    for i in range(len(solvent_smiles)):
        df["solvent" + str(i)] = solvent_smiles[:, i]
...
def load_models(
    ...
    :param load_ghsolv: load the model for solvation free energies and enthalpies
...
def predict_property(
    ...
    Data is read from a csv file or a pandas dataframe.
        ...
        :param ghsolv: predict solvation free energies and enthalpies
...
def calculate_solubility(
    ...
    predict_t_dep = False
        # data.temperatures is not None and (
    ...
    predictions = SolubilityPredictions(
        ...
        predict_solute_parameters=predict_t_dep,
        data=data,
        models=models,
