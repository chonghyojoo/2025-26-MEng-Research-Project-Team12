...
class Splitter:
    ...
    def split_solute(self, data):
        solutes = [i.smiles[0] for i in data]
        solutes = list(set(solutes))
        random.shuffle(solutes)
        train_data, val_data, test_data = self.split(solutes)
        train_data = [i for i in data if i.smiles[0] in train_data]
        val_data = [i for i in data if i.smiles[0] in val_data]
        test_data = [i for i in data if i.smiles[0] in test_data]
        return train_data, val_data, test_data
