...
def evaluate(
    model: nn.Module, data: DatapointList, metric_func: str, scaler: Scaler = None, input: TrainArgs = None
):
    """
    ...
    """
    preds = predict(model=model, data=data, scaler=scaler, inp=input)
...
def predict(model: nn.Module, data: DatapointList, scaler: Scaler = None, inp: TrainArgs = None):
    """
        ...
        for mol in mol_encodings:
            for d in datapoints:
                ...
                if len(encoders) < inp.max_num_mols:
                    for j in range(inp.max_num_mols - len(encoders)):
