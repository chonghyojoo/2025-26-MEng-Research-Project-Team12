...
def train(
    ...
    for i in trange(0, num_iters, iter_size):
        ...
        datapoints = batch.get_data()
        mol_encodings = []
        tensors = []
        for molecule in range(inp.max_num_mols):
        ...
        for mol in mol_encodings:
            for d in datapoints:
                ...
                if len(encoders) < inp.max_num_mols:
                    for i in range(inp.max_num_mols - len(encoders)):
                        encoders.append(encoders[0])
                mol.append(encoders[mol_encodings.index(mol)])
            tensors.append(DataTensor(mol, inp, property=inp.property))
        ...
        if next(model.parameters()).is_cuda or inp.cuda:
            ...
            targets = targets.to(inp.device)
            mask = mask.to(inp.device)

        # Run model
        model.zero_grad()
        preds = model(batch, tensors)
...
# @profile
def run_training(inp: TrainArgs, all_data: DatapointList, logger: Logger):
    """
    ...
    logger(
        f"Number of molecules: {len(all_data[0].smiles)}, features: {len(all_data[0].features)}, "
    ...
    for fold in trange(inp.num_folds):
        ...
        elif inp.split == "solute":
            train_data, val_data, test_data = splitter.split_solute(all_data)
        ...
        for model_i in trange(inp.num_models):
            ...
            logger(f"Number of parameters in model {model_i}: {number_of_parameters}")
            initialize_weights(model, seed=initial_seed + model_i)

            if inp.cuda:
                logger("Moving model to mps or cuda")
                model = model.to(inp.device)

            logger(f"Save checkpoint file to {path}/model{model_i}.pt")
            save_checkpoint(os.path.join(path, "model" + str(model_i) + ".pt"), model, inp, scaler)
            if inp.pretraining and inp.pretraining_path is not None:
                logger(f"Load pretraining parameters from {inp.pretraining_path[fold]}")
                model = load_checkpoint(inp.pretraining_path[model_i], current_inp=inp)
                logger(f"Load scaler from pretrained dataset")
                scaler = load_scaler(inp.pretraining_path[model_i], model_i)
                if inp.scale == "standard":
                    [
            ...
            model = load_checkpoint(os.path.join(path, "model" + str(model_i) + ".pt"), inp, model_i=model_i)
...
def write_ensemble_summary(inp: TrainArgs, fold: int):
...
def load_checkpoint(
    ...
    if from_package:
        ...
        if ".pt" not in path:
            path = path + "/model_" + str(model_i) + ".pt"
    ...
    if args.cuda:
        debug("Moving model to mps or cuda")
...
def process_results(
    ...
    train_preds = predict(model=model, data=data[0], scaler=scaler, inp=input)
    val_preds = predict(model=model, data=data[1], scaler=scaler, inp=input)
    test_preds = predict(model=model, data=data[2], scaler=scaler, inp=input)
    for i in range(input.num_targets):
        fig, ax = plt.subplots()
        train_data = DatapointList(data[0].get_data()[0: len(train_preds)])
        targets = [d.targets[i] for d in train_data.get_data()]
        preds = [p[i] for p in train_preds]

        min = np.min([np.min(targets), np.min(preds)])
        max = np.max([np.max(targets), np.max(preds)])
        ax = plt.gca()
        # ax.set_xlim(min, max)
        # ax.set_ylim(min, max)
        ax.plot([d.targets[i] for d in train_data.get_data()], [p[i] for p in train_preds], "b.", alpha=0.5)
        ax.set_aspect("equal", adjustable="box")
        ax.set(xlabel="Experimental" + input.target_headers[i], ylabel="Prediction" + input.target_headers[i])
        # ax.plot([0, 1], [0, 1], transform=ax.transAxes, color="black", linestyle="dashed")
        fig.savefig(os.path.join(path, "train_parity" + str(i) + ".png"))
        plt.close()

        fig, ax = plt.subplots()
        val_data = DatapointList(data[1].get_data()[0: len(val_preds)])
        targets = [d.targets[i] for d in val_data.get_data()]
        preds = [p[i] for p in val_preds]
        min = np.min([np.min(targets), np.min(preds)])
        max = np.max([np.max(targets), np.max(preds)])
        ax = plt.gca()
        # ax.set_xlim(min, max)
        ...
        ax.set(xlabel="Experimental " + input.target_headers[i], ylabel="Prediction " + input.target_headers[i])
        # ax.plot([0, 1], [0, 1], transform=ax.transAxes, color="black", linestyle="dashed")
        fig.savefig(os.path.join(path, "val_parity" + str(i) + ".png"))
        plt.close()

        fig, ax = plt.subplots()
        test_data = DatapointList(data[2].get_data()[0: len(test_preds)])
        targets = [d.targets[i] for d in test_data.get_data()]
        preds = [p[i] for p in test_preds]
        min = np.min([np.min(targets), np.min(preds)])
        max = np.max([np.max(targets), np.max(preds)])
        ax = plt.gca()
        # ax.set_xlim(min, max)
        ...
        ax.set(xlabel="Experimental " + input.target_headers[i], ylabel="Prediction " + input.target_headers[i])
        # ax.plot([0, 1], [0, 1], transform=ax.transAxes, color="black", linestyle="dashed")
        fig.savefig(os.path.join(path, "test_parity" + str(i) + ".png"))
        plt.close()
    ...
    with open(os.path.join(path, "summary.csv"), "w+") as f:
        writer = csv.writer(f)
        row = ["name", "rmse", "mse", "mae"]
        writer.writerow(row)
        names = []
        for i in range(input.num_targets):
            names.append("train" + str(i))
            names.append("val" + str(i))
            names.append("test" + str(i))

        for i in range(0, len(data2)):
            for j in range(0, input.num_targets):
                row = [
                    names[i],
                    rmse(data=data2[i], target_index=j),
                    mse(data=data2[i], target_index=j),
                    mae(data=data2[i], target_index=j),
                ]
                writer.writerow(row)
...
def write_summary(path: str, input: TrainArgs, data: DatapointList, name: str):
    with open(os.path.join(path, name + "_summary.csv"), "w+", encoding="utf-8") as f:
        writer = csv.writer(f)
        row = []
        for i in range(0, input.max_num_mols):
            row.append("inchi" + str(i))
        for i in range(0, input.num_targets):
            row.append("target" + str(i))
        for i in range(0, input.num_targets):
            row.append("prediction" + str(i))
        for i in range(0, input.max_num_mols):
            row.append("molefraction" + str(i))
        ...
        for d in data.get_data():
            inchis = d.get_inchi()
            for i in range(len(inchis), input.max_num_mols):
                inchis.append("None")
            row = (
                [i for i in inchis]
                + [t for t in d.targets]
                + [p for p in d.predictions]
                + [d for d in d.molefracs]
            )
            if input.split == "scaffold":
...
def rmse(data: DatapointList, target_index: int = 0):
    """
    ...
    """
    targets = [d.targets[target_index] for d in data.get_data()]
    preds = [d.predictions[target_index] for d in data.get_data()]
    nans = []
    for i in range(len(targets)):
        if np.isnan(targets[i]):
            nans.append(i)
    targets = [targets[i] for i in range(len(targets)) if i not in nans]
    preds = [preds[i] for i in range(len(preds)) if i not in nans]
    return math.sqrt(mean_squared_error(targets, preds))


def mse(data: DatapointList, target_index: int = 0) -> float:
    """
...
def mae(data: DatapointList, target_index: int = 0) -> float:
    """
    Computes the mean squared error.
...
def load_scaler(path: str, model_i=0, from_package=False) -> Scaler:
    if from_package:
        state = torch.load(
            io.BytesIO(pkgutil.get_data("solvation_predictor", path)),
            map_location=lambda storage, loc: storage,
...
def load_input(path: str) -> TrainArgs:
