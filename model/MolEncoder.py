from features.feature_vectors import *
...
class MolEncoder:
    ...
    def __init__(self, mol: Chem.rdchem.Mol, property: str = "solvation", dummy=False):
        if not isinstance(mol, Chem.Mol):
            raise ValueError(f"[MolEncoder] Expected Chem.Mol, got {type(mol)}")
        ...
        self.b2a = b2a
        self.b2revb = b2revb
        self.ascope = [(0, len(self.f_atoms))]
        self.bscope = [(0, len(self.f_bonds))]


    def get_components(self):
        return {
            'f_atoms': self.f_atoms,
